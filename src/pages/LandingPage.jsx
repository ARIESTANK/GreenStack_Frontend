...
const LandingPage = () => {
  ...
  <div className="bg-[#C5A677] rounded-[2rem] md:rounded-[3rem] p-8 md:p-16 text-white relative flex flex-row items-center justify-between overflow-hidden shadow-xl min-h-[220px] md:min-h-[450px]">
    ...
    <div className="w-[65%] md:w-1/2 z-20">
      <h2 className="text-xl md:text-5xl font-black leading-tight mb-3 md:mb-6 tracking-tight">
        {t.heroSection.headlineMain} <br className="hidden md:block" />{" "}
        <span className="text-[#3F865F] italic font-black">
          {t.heroSection.headlineItalic}
        </span>
      </h2>
      
      <p className="text-[11px] md:text-xl font-medium opacity-95 mb-5 md:mb-10 max-w-md leading-relaxed">
        {t.heroSection.description}
        <span className="font-bold text-[#3F865F]">
          {t.heroSection.descriptionBold}
        </span>
        {/* Render descriptionEnd only if it exists (for Myanmar structure) */}
        {t.heroSection.descriptionEnd && <span>{t.heroSection.descriptionEnd}</span>}
      </p>

      <button onClick={handleUser} className="bg-[#3F865F] hover:bg-[#2d6346] text-white px-5 py-2.5 md:px-10 md:py-5 rounded-xl md:rounded-[2rem] font-black text-[10px] md:text-lg shadow-lg flex items-center gap-2 transition-all hover:scale-105 active:scale-95">
        {t.heroSection.buttonText} <ChevronRight size={18} />
      </button>
    </div>

    {/* Character/Mockup Area */}
        ...
        <div className="bg-[#3F865F] rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-16 text-white relative flex flex-col md:flex-row items-center justify-between overflow-hidden shadow-2xl shadow-[#3F865F]/30 min-h-[450px]">
          ...
          <div className="w-full md:w-1/2 z-20 text-center md:text-left">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full mb-6 border border-white/10">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#A3C475]">{t.aiBadge}</span>
            </div>
            
            <h2 className="text-3xl md:text-6xl font-black leading-tight mb-6 tracking-tight">
               {t.aiTitle1} <br/> 
                <span className="text-[#A3C475]">{t.aiTitle2}</span>
            </h2>
            
            <p className="text-sm md:text-xl font-medium opacity-90 mb-10 max-w-md leading-relaxed">
                {t.aiDesc} <span className="text-[#C5A677] font-bold underline decoration-2">{t.soiltype}</span> {t.getkk}
            </p>
    ...
    <div className="relative group">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-2xl shadow-black/20 border-l-[10px] border-[#A3C475] relative z-20 transform -rotate-2 group-hover:rotate-0 transition-transform duration-500">
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    {/* Key: aiBadge */}
                    <p className="text-[10px] font-black text-[#C5A677] uppercase tracking-widest leading-none mb-1">
                        {t.chatSection.aiBadge}
                    </p>
                    {/* Key: soilType */}
                    <h4 className="text-sm font-black text-[#3F865F]">
                        {t.chatSection.soilType}
                    </h4>
                </div>
            </div>
        ...
        <div className="absolute -top-12 -left-6 md:-left-12 bg-[#C5A677] text-white p-4 rounded-2xl rounded-bl-none shadow-xl z-10 hidden sm:block animate-bounce duration-[3s]">
            <div className="flex items-center gap-2">
                ...
                {/* Key: userQuestion */}
                <span className="text-xs font-bold">{t.chatSection.userQuestion}</span>
            </div>
        </div>
  ...
  <div className="bg-white border-2 border-[#A3C475]/20 rounded-[2rem] md:rounded-[3rem] p-8 md:p-16 flex flex-col md:flex-row items-center gap-12 shadow-sm">
    ...
    <div className="w-full md:w-1/2 relative flex justify-center items-center h-64 md:h-auto">
      ...
      <div className="relative z-10 bg-white border border-gray-100 shadow-2xl p-6 rounded-[2rem] w-64 md:w-72 rotate-[-3deg] hover:rotate-0 transition-transform duration-500">
          <div className="flex justify-between items-center mb-4">
              <span className="bg-[#3F865F]/10 text-[#3F865F] px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider italic">
                {t.marketSection.paddyPrice}
              </span>
              <TrendingUp size={18} className="text-[#3F865F]" />
          </div>
          <div className="flex items-center gap-2 text-gray-400 text-[10px] font-bold mb-1">
              <MapPin size={12} className="text-[#C5A677]" /> {t.marketSection.location}
          </div>
          <h3 className="text-2xl font-black text-[#3F865F] mb-4">
            2,850,000 <small className="text-xs">{t.marketSection.currency}</small>
          </h3>
      ...
      <div className="absolute top-10 right-4 md:right-10 bg-[#C5A677] text-white p-4 rounded-2xl shadow-xl w-32 md:w-40 rotate-[6deg] z-20">
          <p className="text-[8px] font-black uppercase tracking-widest opacity-80 mb-1">
            {t.marketSection.brokerTitle}
          </p>
          <p className="text-xs font-bold leading-none">
            {t.marketSection.brokerName}
          </p>
    ...
    <div className="w-full md:w-1/2 text-center md:text-left">
      <div className="inline-flex items-center gap-2 bg-[#A3C475]/20 px-4 py-2 rounded-full mb-6">
          ...
          <span className="text-[10px] font-black text-[#3F865F] uppercase tracking-[0.2em]">
            {t.marketSection.badge}
          </span>
      </div>
      <h2 className="text-3xl md:text-5xl font-black text-[#3F865F] leading-tight mb-6 italic">
          {t.marketSection.headlineMain} <br/> 
          <span className="text-[#C5A677]">{t.marketSection.headlineSub}</span>
      </h2>
      <p className="text-gray-500 font-medium mb-8 leading-relaxed max-w-md">
          {t.marketSection.description}
      </p>
