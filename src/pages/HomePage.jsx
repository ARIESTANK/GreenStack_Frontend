...
const HomePage = () => {
  ...
  const route="https://firebase-server-database-testing-production.up.railway.app"
  ...
  try {
    ...
    if (data.response?.forecast) {
      const dynamicForecast = data.response.forecast.map((item, index) => ({
        ...
        temp: Math.round(item.temp_max).toString()+"-"+Math.round(item.temp_min).toString(),
        humidity:Math.round(item.humidity).toString(),
        rainFall:(item.rainfall).toString(),
        pressure:item.pressure,
        uv:item.uv_index,
        windDir:item.wind_direction||150,
          ...
          <div className="relative z-10 mb-10">
            <div className="flex md:grid md:grid-cols-4 lg:grid-cols-7 overflow-x-auto md:overflow-visible no-scrollbar gap-4 pb-4 md:pb-0">
              ...
              <WeatherMetric
                icon={<Compass />}
                label="Wind Dir"
                value={farmer.weeklyForecast[0]?.wind_direction || "150" + " °"}
         />
