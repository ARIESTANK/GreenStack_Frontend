...
const Navbar = (props) => {
  ...
  const askQuestion = async () => {
    ...
    const res = await fetch(
      "https://firebase-server-database-testing-production.up.railway.app/chatBot",
      {
        method: "POST",
        headers: {
  ...
  const handleSendSoil = async () => {
    ...
    if (selectedSoil) {
      const response = await fetch(
        "https://firebase-server-database-testing-production.up.railway.app/cropRecommendation",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <>
      ...
      <nav className="hidden md:flex sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 px-12 py-4 justify-between items-center shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-black text-[#3F865F] tracking-tight">
            GreenStack
          </span>
        </div>
        ...
        <div className="flex gap-10 font-bold text-sm uppercase tracking-widest">
          <a
            ...
            className={`relative pb-1 transition-all ${
              active === "Home" ? "text-[#3F865F]" : "text-[#3F865F]/60"
            }`}
          >
            Home
          </a>
          <a
            ...
            onClick={() => {
              ...
              setActive("Market Price");
            }}
            className={`relative pb-1 transition-all ${
              active === "Market Price" ? "text-[#3F865F]" : "text-[#3F865F]/60"
            }`}
          >
            Market Price
          </a>
          ...
          <button
            ...
            className={`relative pb-1 transition-all uppercase font-bold text-sm tracking-widest ${
              active === "Crop Tips" ? "text-[#3F865F]" : "text-[#3F865F]/60"
            }`}
          >
            Crop Tips
            {active === "Crop Tips" && (
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-full font-bold shadow-lg transition-all duration-300 ${
              active === "Profile"
                ? "bg-[#3F865F] text-white"
                : "bg-[#C5A677] text-white"
            }`}
          >
          ...
          <button
            onClick={() => setIsChatOpen(true)}
            className="p-2 rounded-full bg-[#A3C475]/20 text-[#3F865F] hover:scale-110 transition-all"
          >
            <Bot size={22} />
          </button>
        </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full px-4 pb-4 z-[55]">
        <div className="relative bg-white h-20 rounded-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.12)] flex items-center justify-around px-2 border border-gray-100">
          ...
          {/* 2. Crop Tips (This triggers your sidebar state) */}
          <MobileNavItem
            icon={<Sprout size={28} />}
            active={isTipsOpen}
          ...
          {/* 3. Central Chatbot Button (Triggers sidebar state) */}
          <div className="absolute -top-10 left-1/2 -translate-x-1/2">
            <div className="w-20 h-20 bg-[#FDFEFA] rounded-full flex items-center justify-center">
              <button
                onClick={() => setIsChatOpen(true)}
                className="w-16 h-16 rounded-full flex items-center justify-center text-white shadow-xl border-4 border-white bg-[#3F865F] shadow-[#3F865F]/40 active:scale-90 transition-all"
              >
                <Bot size={32} />
              </button>
            </div>
          ...
          {/* 4. Feedback / Store Page */}
          <MobileNavItem
            icon={<Store size={28} />}
            active={isActive("/marketprice")}
      ...
      <div
        className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white z-[70] shadow-2xl transition-transform duration-500 ease-in-out transform ${
        ...
        <div className="bg-[#3F865F] p-6 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold text-[#A3C475] tracking-widest mt-1">
                Ready to Help
              </p>
            </div>
          </div>
        ...
        <div className="h-[calc(100vh-180px)] overflow-y-auto p-6 bg-[#FDFEFA] space-y-4 flex flex-col">
          ...
          <div className="bg-white border border-gray-100 p-4 rounded-2xl rounded-tl-none shadow-sm max-w-[85%]">
            <p className="text-sm font-medium text-gray-700">
              မင်္ဂလာပါ။ Green Assistant မှ ကြိုဆိုပါတယ်။
              စိုက်ပျိုးရေးနှင့်ပတ်သက်သည်များကို မေးမြန်းနိုင်ပါသည်။
            </p>
          </div>
      ...
      <div
        className={`fixed inset-0 bg-black/40 backdrop-blur-sm z-[60] transition-opacity duration-300 ${
          isTipsOpen ? "opacity-100 visible" : "opacity-0 invisible"
        }`}
        onClick={closeSidebars}
      />
      <div
        className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white z-[70] shadow-2xl transition-transform duration-500 ease-in-out transform ${
          isTipsOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        {/* Header */}
        <div className="bg-[#C5A677] p-6 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold opacity-80 mt-1 italic">
                Based on Soil Type
              </p>
            </div>
          </div>
        ...
        <div className="h-[calc(100vh-180px)] overflow-y-auto p-6 space-y-6 bg-[#FDFEFA]">
          <div className="bg-white border border-gray-100 p-4 rounded-2xl shadow-sm">
            <p className="text-sm font-bold text-[#3F865F] mb-4">
              What type of soil do you have in your field?
            </p>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {tipsResponse.map((item, index) => (
                ...
                >
                  {/* Rank Badge */}
                  <div
                    className={`absolute top-0 right-0 px-4 py-1 rounded-bl-xl text-[10px] font-bold text-white ${
              ...
              <p className="text-[10px] text-center text-gray-400 font-medium px-4">
                Recommendations based on your local soil NPK, real-time weather,
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 bg-white border-t border-gray-100">
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-sm shadow-lg flex items-center justify-center gap-2 transition-all ${
              selectedSoil
                ? "bg-[#3F865F] text-white shadow-[#3F865F]/20"
...
const MobileNavItem = ({ icon, active, onClick }) => (
  ...
  >
    ...
    {active && (
      <div className="w-1.5 h-1.5 bg-[#3F865F] rounded-full mt-1"></div>
    )}
  </button>
);
